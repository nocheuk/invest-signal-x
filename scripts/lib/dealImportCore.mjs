...
function asOption(value, allowed, fallback) {
    if (!value)
        return fallback;
    const match = allowed.find((item) => item.toLowerCase() === value.toLowerCase());
    return match ?? fallback;
}
export function extractPostcode(location, explicitPostcode) {
    if (explicitPostcode?.trim())
        return explicitPostcode.trim().toUpperCase();
    const full = location.match(/[A-Z]{1,2}\d[A-Z\d]?\s*\d[A-Z]{2}/i);
    if (full)
        return full[0].toUpperCase().replace(/\s+/, " ");
...
export function buildDedupeKeys(row) {
...
export function parseDealCsv(csv) {
...
export function normalizeImportRow(rawInput, rowNumber = 1) {
...
export function validateImportRow(row) {
    ...
    if (!row.guidePrice || row.guidePrice <= 0)
        errors.push("guide_price must be greater than 0");
    else if (!isSafeGuidePrice(row.guidePrice))
        errors.push(`guide_price must be a safe integer between ${MIN_GUIDE_PRICE} and ${MAX_GUIDE_PRICE}`);
    if (row.assetType && !ASSET_TYPES.includes(row.assetType))
...
export function isSafeGuidePrice(value) {
    return Number.isSafeInteger(value) && value >= MIN_GUIDE_PRICE && value <= MAX_GUIDE_PRICE;
}
function isSafeNonNegativeNumber(value, max = MAX_GUIDE_PRICE) {
    return Number.isSafeInteger(value) && value >= 0 && value <= max;
}
export function findDuplicate(row, existingDeals) {
...
export function mapImportToDealInsert(row, sourceName = "Manual import") {
    ...
    const score = Math.round(scoreBreakdown.incomeQuality * 0.25 +
...
export function importRowsToCsv(rows) {
...
function importRowIdentityKey(row) {
    if (row.normalized.externalId) {
        ...
        return { identity: `external_id:${value}`, rule: "external_id", value };
    }
    if (row.normalized.sourceUrl) {
        const value = normalizeText(row.normalized.sourceUrl);
        return { identity: `source_url:${value}`, rule: "source_url", value };
    }
    const fallback = row.dedupeKeys.titlePostcode || row.dedupeKeys.titlePriceLocation;
    if (!fallback)
        return null;
    return { identity: `dedupe_key:${fallback}`, rule: "dedupe_key", value: fallback };
}
...
function defaultVoidRisk(value) {
    if (value === "Strong")
        return 12;
    if (value === "Good")
        return 22;
